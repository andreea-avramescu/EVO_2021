...
from radiuses import createManuSol
from coverage import stochastic_coverage
from coverage import full_locations
...
def append_CryoSol(population,manu_list):
    ...
    uncovered_set = set(range(len(hospitals_list)))
    for manu in manu_list:
        uncovered_set -= hosp_for_manu[manu]  # for each manu, it removes the set of covered hospitals
    ...
    # exclude locations occupied by manufacturing
    [uncovered_dict.pop(key) for key in manu_list]

    full_coverage_cryo = stochastic_coverage(uncovered_dict)
    
    if non_zero_coverage(manu_list):
        population.append(Solution(manu_list,[]))

    for stage in range(len(full_coverage_cryo)):
        # append solution with different levels of coverage from empty list [] to full_coverage_cryo list
        cryo_list = full_coverage_cryo[:stage+1]

        population.append(Solution(manu_list,cryo_list))
...
def initialPopulation(popSize):
    ...
    while len(population) < popSize:
        # Generate random manufacturing solution
        manu_list = createManuSol()
        # Append all cryo solutions
        append_CryoSol(population,manu_list)
        
    return population

def dominates(obj1, obj2, sign=[-1, -1, -1]):
...
def sortNondominated(fitness, nr_parents=None, first_front_only=False):
    """Sort the first *nr_parents* *individuals* into different nondomination levels
        using the "Fast Nondominated Sorting Approach" proposed by Deb et al.,
        see [Deb2002]_. This algorithm has a time complexity of :math:`O(MN^2)`,
        where :math:`M` is the number of objectives and :math:`N` the number of
        individuals.
        :param individuals: A list of individuals to select from.
        :param nr_parents: The number of individuals to select.
    ...
    """
    if nr_parents is None:
        nr_parents = len(fitness)
    ...
    if not first_front_only:  # first front only
        ...
        while pareto_sorted < N:
            ...
            current_front = copy.deepcopy(next_front)
            next_front = []

    return fronts

def CrowdingDist(population,front,cut):
    """
    :param fitness: A list of fitness values
    :return: A list of crowding distances of chrmosomes
    
    The crowding-distance computation requires sorting the population according to each objective function
    value in ascending order of magnitude. Thereafter, for each objective function, the boundary solutions
    (solutions with smallest and largest function values) are assigned an infinite distance value. All
    other intermediate solutions are assigned a distance value equal to the absolute normalized difference
    in the function values of two adjacent solutions.
    """

    # initialize list: [0.0, 0.0, 0.0, ...]
    distances = np.full(len(front),0.0)
    fitness = [(population[i].avgwait_time,population[i].total_cost,population[i].uncovered) for i in front]
    
    crowd = [(f_value, i) for i, f_value in enumerate(fitness)]  # create keys for fitness values

    n_obj = len(fitness[0])  # number of objective functions (3 for FLP)
    ...
    a_front = np.array(front)
    return a_front[np.argsort(distances)[-cut:]]

# return the values of the objective functions for each solution in the population
def fitnessSolutions(population):
    fitnessResults = []
    for sol in population:
        sol.calc_objectives()
        fitnessResults.append((sol.avgwait_time,sol.total_cost,sol.uncovered))
    return fitnessResults

def selection(population, fronts, nr_parents):
    selectionResults = []
    
    i = 0
    while len(selectionResults) + len(fronts[i]) <= nr_parents:
        selectionResults.extend(fronts[i])
        i += 1
    
    selectionResults.extend(CrowdingDist(population,fronts[i], nr_parents - len(selectionResults)))
    
    return selectionResults

# define the mating function
def breed(set1, set2):    
    # 2 parents with sets of manu solutions
    child = list(set1 & set2)
    
    # if there is a common manufacturing location, the location is kept as child
        # the non-identical locations are appended to the child with 0.5 probability
            # empty manu child solutions are not allowed
    for gene in list((set1 | set2) - (set1 & set2)):
        r = random.random()
        if r<0.5:
            child.append(gene)
    
    if child == []:
        child = random.sample(list((set1 | set2)), 1)
...
def breedPopulation(population):
    ...
    pool = random.sample(population, population_size)
    # parent 1 is fixed; if parent 2 has the same manu_list locactions,
        # parent 2 becomes random
    i = 0
    while len(population) < population_size*2:
        parent1 = set(population[i].manu_list)
        parent2 = set(pool[i].manu_list)
        
        # ensure parents have different manufacturing solutions
        while parent1 - parent2 == set() or parent2 - parent1 == set():
            parent2 = set(random.sample(pool, 1)[0].manu_list)
        
        child = breed(parent1, parent2)
        # Append all cryo solutions
        append_CryoSol(population,child)
        i += 1

# define the mutation function
    # 1. with 0.1 probability add a random manufacturing;
    # 2. with 0.1 probability remove a random manufacturing
        # empty manufacturing solutions are not allowed;
    # 3. with 0.8 probability move a random manufacturing in an location from the network road;

def mutate(individual, add = 0.1, remove = 0.1, mutation_network = road_network):
    mutation = copy.deepcopy(individual.manu_list)
    
    r = random.random()
    if r < add:
        new_manu = random.randint(0,len(locations)-1)
        
        # ensure that new_manu is not already in manu_list(mutation)
        while new_manu in mutation:
            new_manu = random.randint(0,len(locations)-1)
        
        mutation.append(new_manu)
        
    elif r < add + remove and len(mutation) > 1:
        rem_manu = random.sample(mutation, 1)
        
        mutation.remove(rem_manu[0])
    else:
        manu = random.sample(mutation, 1)
        
        adjacent_manu = mutation_network[manu[0]]
        
        adj_manu = random.sample(adjacent_manu, 1)
        
        # ensure that adj_manu is not already in manu_list(mutation)
        while adj_manu[0] in mutation:
            adj_manu = random.sample(adjacent_manu, 1)
        
        mutation.remove(manu[0])
        mutation.append(adj_manu[0])
...
def mutatePopulation(population):
    ...
    while len(population) < population_size*2:
        ...
        # Append all cryo solutions
        append_CryoSol(population,mutation)
        i += 1

all_fitness = []

def nextGeneration(currentGen, nr_parents):
    fitness = fitnessSolutions(currentGen)
    
    fronts = sortNondominated(fitness, nr_parents)

    fitness_array = np.array(fitness)  # added
    all_fitness.append(fitness_array[fronts[0]])   # added
    
    parents_list = selection(currentGen, fronts, nr_parents)
    nextGeneration = [currentGen[index] for index in parents_list]
    
    breedPopulation(nextGeneration)
    mutatePopulation(nextGeneration)
    return nextGeneration

def NSGAII(generations, popSize = POPULATION, nr_parents = PARENTS):
    pop = initialPopulation(popSize)

    for i in range(generations):
        pop = nextGeneration(pop, nr_parents)
        print(i, end = ' ')
    print()
    
    fitness = fitnessSolutions(pop)
    pareto_front_indexes = sortNondominated(fitness, first_front_only=True)[0]
    
    fitness_array = np.array(fitness)  # added
    all_fitness.append(fitness_array[pareto_front_indexes])   # added
    
    pareto_front_sol = []
    
    for sol_index in pareto_front_indexes:
        pareto_front_sol.append([pop[sol_index].manu_list,pop[sol_index].cryo_list])
    
    return pareto_front_sol